...
function abrirModal(modal, metodo, id) {
    ...
    $('#id_tipo_atividade').val(id);
    if (metodo == 'atualizar') {
        $('#metodo').val('getTipo_atividadeId');
        carregar(id);
    } else {
...
function carregar() {
    ...
    $.ajax({
        ...
        $("#id_categoria").val(json.descricao);
        $("#descricao").val(json.descricao);
        $("#ch_maxima").val(json.descricao);
        $("#ch_minima").val(json.descricao);
    });
    
    function carregarCategoria(){
        $('#metodo').val('carregarCategoria');
        var dados = $('#formulario').serialize();
        $.ajax({
            url: '../controller/' + arquivo(),
            type: 'post',
